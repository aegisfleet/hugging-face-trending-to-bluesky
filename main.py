...
def main():
    ...
    targets = huggingface_utils.get_trending_hf_models()

    gpt_client = GPTClient()
    bs_client = BSClient()

    for full_url, href, repo_name in targets:
        print(f"\nURL: {full_url}\nName: {repo_name}")

        readme_text = huggingface_utils.get_readme_text(href)
        retries = 0
        max_retries = 3
        while retries < max_retries:
            message = gpt_utils.get_description(gpt_client, href, readme_text)
            title, description, image_url = bluesky_utils.fetch_webpage_metadata(full_url)
            post_text = bluesky_utils.format_message_with_link(repo_name, full_url, "今日のHuggingFaceトレンド", message)
